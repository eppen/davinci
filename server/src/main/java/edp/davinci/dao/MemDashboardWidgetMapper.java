...
/**
 * MyBatis mapper. XML: server/src/main/resources/mybatis/mapper/MemDashboardWidgetMapper.xml
 * XML statements: insert, insertBatch, updateBatch.
 */
@Component
public interface MemDashboardWidgetMapper {
    ...
    @Update(value = {"update mem_dashboard_widget", "set alias = #{alias,jdbcType=VARCHAR},", "dashboard_id = #{dashboardId,jdbcType=BIGINT},", "widget_Id = #{widgetId,jdbcType=BIGINT},", "x = #{x,jdbcType=INTEGER},", "y = #{y,jdbcType=INTEGER},", "width = #{width,jdbcType=INTEGER},", "height = #{height,jdbcType=INTEGER},", "polling = #{polling,jdbcType=BIT},", "frequency = #{frequency,jdbcType=INTEGER},", "[config] = #{config,jdbcType=LONGVARCHAR}", "where id = #{id,jdbcType=BIGINT}"}, databaseId = "sqlserver")
