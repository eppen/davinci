...
/**
 * MyBatis mapper. XML: server/src/main/resources/mybatis/mapper/MemDisplaySlideWidgetMapper.xml
 * XML statements: insert, insertBatch, updateBatch, deleteBatchById.
 */
@Component
public interface MemDisplaySlideWidgetMapper {
    ...
    @Update(value = {"update mem_display_slide_widget", "set [display_slide_id] = #{displaySlideId,jdbcType=BIGINT},", "widget_id = #{widgetId,jdbcType=BIGINT},", "[name] = #{name,jdbcType=VARCHAR},", "[type] = #{type,jdbcType=SMALLINT},", "sub_type = #{subType,jdbcType=SMALLINT},", "[index] = #{index,jdbcType=INTEGER},", "[params] = #{params,jdbcType=LONGVARCHAR},", "update_by = #{updateBy,jdbcType=BIGINT},", "update_time = #{updateTime,jdbcType=TIMESTAMP}", "where id = #{id,jdbcType=BIGINT}"}, databaseId = "sqlserver")
    ...
    @Select(value = {"SELECT m.*,", "	s.id 'displaySlide.id',", "	s.display_id 'displaySlide.displayId',", "	s.[index] 'displaySlide.index',", "	s.[config] 'displaySlide.config'", "FROM mem_display_slide_widget m LEFT JOIN display_slide s on m.display_slide_id = s.id", "WHERE s.display_id = #{displayId}"}, databaseId = "sqlserver")
