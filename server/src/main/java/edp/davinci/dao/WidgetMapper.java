...
/**
 * MyBatis mapper. XML: server/src/main/resources/mybatis/mapper/WidgetMapper.xml
 * XML statements: insert, insertBatch, updateConfigBatch, getByIds, getIdSetByIds.
 */
@Component
public interface WidgetMapper {
    ...
    @Update(value = {"update widget", "set [name] = #{name,jdbcType=VARCHAR},", "[description] = #{description,jdbcType=VARCHAR},", "[view_id] = #{viewId,jdbcType=BIGINT},", "[project_id] = #{projectId,jdbcType=BIGINT},", "[type] = #{type,jdbcType=BIGINT},", "[publish] = #{publish,jdbcType=BIT},", "[config] = #{config,jdbcType=LONGVARCHAR},", "[update_by] = #{updateBy,jdbcType=BIGINT},", "[update_time] = #{updateTime,jdbcType=TIMESTAMP}", "where id = #{id,jdbcType=BIGINT}"}, databaseId = "sqlserver")
    ...
    @Select(value = {"select id from widget where project_id = #{projectId} and `name` = #{name}"}, databaseId = "mysql")

    @Select(value = {"select id from widget where project_id = #{projectId} and [name] = #{name}"}, databaseId = "sqlserver")
