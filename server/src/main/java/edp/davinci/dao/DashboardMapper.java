...
/**
 * MyBatis mapper. XML: server/src/main/resources/mybatis/mapper/DashboardMapper.xml
 * XML statements: insert, updateBatch, queryByParentIds, getIdSetByIds, queryDashboardsByIds, queryByPortals.
 */
@Component
public interface DashboardMapper {
    ...
    @Select(value = {"select id from dashboard where dashboard_portal_id = #{portalId} and `name` = #{name}"}, databaseId = "mysql")

    @Select(value = {"select id from dashboard where dashboard_portal_id = #{portalId} and [name] = #{name}"}, databaseId = "sqlserver")
    ...
    @Update(value = {"update dashboard", "set [name] = #{name,jdbcType=VARCHAR},", "dashboard_portal_id = #{dashboardPortalId,jdbcType=BIGINT},", "[type] = #{type,jdbcType=SMALLINT},", "[index] = #{index,jdbcType=INTEGER},", "parent_id = #{parentId,jdbcType=BIGINT},", "[config] = #{config,jdbcType=LONGVARCHAR},", "update_by = #{updateBy,jdbcType=BIGINT},", "update_time = #{updateTime,jdbcType=TIMESTAMP}", "where id = #{id,jdbcType=BIGINT}"}, databaseId = "sqlserver")
    ...
    @Select(value = {"select * from dashboard where dashboard_portal_id = #{portalId} order by [index]"}, databaseId = "sqlserver")
