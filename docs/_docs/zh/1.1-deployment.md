...
1. 创建数据库（如 `davinci`），使用 SSMS 或 sqlcmd 导入全量脚本：

```bash
sqlcmd -S localhost -U sa -P your_password -d davinci -i $DAVINCI3_HOME/bin/davinci.sqlserver.sql
```

2. 或配置环境变量后执行 `bin/initdb-sqlserver.sh`（需安装 sqlcmd）。

3. 在 `config/application.yml` 中配置 SQL Server 数据源，例如：

```yaml
spring:
  datasource:
    url: jdbc:sqlserver://localhost:1433;databaseName=davinci;encrypt=true;trustServerCertificate=true
    username: sa
...
已有 MySQL 环境升级至新版本时，请使用 `bin/patch/` 下对应补丁；SQL Server 使用 `bin/patch-sqlserver/` 中同名脚本。
